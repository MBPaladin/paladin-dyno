...
torque ripple is present on top of this baseline (RMS
$\approx\SI{1.75}{\newton\meter}$ at top speed, input-referred), at the cycloidal
lobe-pass frequency; it is the dominant high-speed feature, and its spectral
make-up is examined in the frequency-content analysis below
(Figure~\ref{fig:gbrun-fft}).
...
Because of motor speed limitations, the output-driven sweep reaches far higher output speed (\SI{10.3}{\radian\per\second}
vs.\ \SI{2.4}{}) and so exercises the viscous term much more strongly. The two runs
...
% ===========================================================================
The stationary, unpowered dynamometer was struck with a rubber mallet and the
...
the low frequency \SI{21.4}{\hertz} mode lines up with the running-torque ripple
resonance ($\approx\SI{21.4}{\hertz}$, Section~\ref{sec:gbrunning}): the ripple
amplified at high input speed is partly a \emph{structural} resonance of the test
fixture being driven by the cycloidal lobe-pass forcing, not solely a property of
the gearbox itself. This should be kept in mind when interpreting the high-speed
ripple magnitude.
