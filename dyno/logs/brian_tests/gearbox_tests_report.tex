...
\resultfig{pics/gearboxCropped.png}{0.4}{Cycloidal-drive gearbox design that was tested (Note: Two units were supplied for testing. This report outlines the results for Gearbox 1, while the image depicts Gearbox 2.)}{fig:gearbox}
...
\begin{table}[H]
  ...
  \begin{tabular}{lr}
    ...
    Quantity & Value\\
    \midrule
    Coulomb drag         & \SI{0.044}{\newton\meter}\\
    Viscous drag         & \SI{0.0289}{\newton\meter\per(\radian\per\second)}\\
    Drag at max speed (\SI{2.4}{\radian\per\second}) & \SI{0.11}{\newton\meter}\\
    \bottomrule
  \end{tabular}
\end{table}
...
and adds noise in the \SIrange{1}{2}{\radian\per\second} regime, but that growth is mostly the test
fixture's structural resonance being excited rather than the gearbox itself (see
Figure~\ref{fig:gbrun-fft}). The ripple magnitude is quantified under load below
...
the power-equivalent factor of 26 ($0.044\times26\approx\SI{1.2}{\newton\meter}$)
...
\resultfig{gearbox_tests/results/output_running_torque/output_running_torque_fft_vs_command.png}{0.85}
  {Output-driven running-torque spectrum (top, segment-averaged) and spectrogram with respect to commanded input velocity magnitude
   (bottom).}{fig:gbrun-fft}
...
the cycloidal lobe-pass forcing and other periodic mechanical effects. Riding on top of these is a
broad, \emph{speed-independent} peak near \SI{17}{\hertz} in the averaged
...
coincides with the ($17\times26\approx\SI{440}{\Hz}$) fundamental from the tap test
(Appendix~\ref{sec:tap}), with the lobe-pass forcing ringing the fixture as it
sweeps through.

Using spectrograms in this way, we can test preloads, look at balances and tolerances, and identify where in the cycloid mechanical imperfections arise.
