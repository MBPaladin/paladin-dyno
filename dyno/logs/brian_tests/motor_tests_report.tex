...
\begin{itemize}
  ...
  \item \textbf{Mid/high-frequency excitation.} Because cogging repeats once per
        magnetic period (i.e. once per pole), its forcing frequency is $60\times$ the mechanical
        ...
        resonant amplification of the ripple (Section~\ref{sec:cogging}).
...
\paragraph{Per-phase (local) view.} The ripple across the mechanical angle is periodic at \SI{60} oscillations per mechanical revolution, representing the pole count.
Folding over the single-pole period and
subtracting the mean isolates the repeatable cogging waveform. The peak-to-peak
ripple is $\approx\SI{1.1}{\newton\meter}$ unloaded, rising to
$\approx\SI{1.6}{\newton\meter}$ under load, a
load dependence expected when stator saturation interacts with the rotor detent pattern.
Turning this motor into a perfect torque source would involve two feedforward compensations: (i) a static, per-pole-period cogging map and (ii) a torque-dependent gain to counteract the load-induced growth.

\resultfig{motor_tests/results/cogging_10s/cogging_10s_local.png}{0.72}
  {Phase-folded, mean-subtracted cogging over one pole period at
...
$n\,p\,\omega/\pi$ for $p=30$ pole pairs), whereas a structural resonance stays at
...
the time-domain puzzle: at \SI{24}{\radian\per\second} the first cogging
order ($p\,\omega/\pi \approx \SI{229}{\hertz}$) lands on the \SI{228}{\hertz}
mode and drives the large ripple, while the weaker bump near
\SI{12}{\radian\per\second} is the second order crossing the same mode. Note that
...
\section{Conclusion} The motor's intrinsic cogging is relatively well-behaved:
$\approx\SI{1.1}{\newton\meter}$ peak-to-peak unloaded, growing modestly under
load. Ideal cogging compensation would be a feedforward function that accounts for both the static per-pole-period cogging map and a torque-dependent compensator that adjusts for higher-load effects.

The dramatic ripple at certain speeds is not a magnetic property but a
\emph{test-rig} effect, a $\approx\SI{228}{\hertz}$ structural resonance that the pole-pass harmonic excites near \SI{24}{\radian\per\second}. On a different mechanical
