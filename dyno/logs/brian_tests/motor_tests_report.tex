...
\author{Paladin Dynamics --- Agility Dyno}
\date{\today}
...
high--pole-count machine with \textbf{30 pole pairs}, driven on the Agility dyno
...
saturating model with a zero-current bias term,
\begin{equation}
  T(I) = A\,\tanh(B\,I) + C,
\end{equation}
where the $\tanh$ captures magnetic saturation and $C$ absorbs the measured
zero-current torque bias (cell offset, cogging at the parked angle, gravity).
$C$ is reported as a diagnostic and then subtracted, so the plotted data, fit,
and datasheet overlay all share an odd, through-origin frame.
...
\begin{table}[H]
  ...
  \begin{tabular}{lrl}
    ...
    Zero-current bias $C$   & \num{0.797}\,\si{\newton\meter} & removed after fit\\
    $R^2$ / RMSE            & \num{0.9997} / \num{0.71}\,\si{\newton\meter} & over $\pm\,\SI{11.4}{\ampere_{rms}}$\\
    \bottomrule
  \end{tabular}
\end{table}

\paragraph{Fit vs.\ spec sheet.} The fit is essentially exact ($R^2=0.9997$).
...
the load cell recorded the torque needed to turn it --- pure parasitic loss. The
